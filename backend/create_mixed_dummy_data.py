...
def create_dummy_data_sqlite():
    """Create dummy data for SQLite database"""
    ...
    try:
        ...
        # Clear existing data
        cursor.execute("DELETE FROM ride_history")
        cursor.execute("DELETE FROM messages")
        cursor.execute("DELETE FROM ride_requests")
        cursor.execute("DELETE FROM rides")
        cursor.execute("DELETE FROM schedules")
        cursor.execute("DELETE FROM preferred_locations")
        cursor.execute("DELETE FROM cars")
        cursor.execute("DELETE FROM users")
        
        # Insert users
        cursor.executemany("""
            INSERT INTO users (id, name, email, phone, photo_url, bio, is_driver, is_rider, trust_score, preferences)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, users_data)
        
        # Insert cars
        cursor.executemany("""
            INSERT INTO cars (id, user_id, make, model, year, color, license_plate, seats, photo_url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, cars_data)
        
        # Insert preferred locations
        cursor.executemany("""
            INSERT INTO preferred_locations (id, user_id, name, address, latitude, longitude)
            VALUES (?, ?, ?, ?, ?, ?)
        """, locations_data)
        ...
        # Insert rides
        cursor.executemany("""
            INSERT INTO rides (id, driver_id, car_id, start_location_id, end_location_id, start_time, seats_available, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, rides_data)
        ...
        for ride in completed_rides:
            ...
            rider_ids = random.sample([user[0] for user in users_data if user[0] != driver_id and user[7]], num_riders)
        ...
        # Insert ride history
        cursor.executemany("""
            INSERT INTO ride_history (id, user_id, ride_id, role, joined_at, completed_at, rating_given, rating_received)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, ride_history_data)
        ...
        print(f"Successfully created dummy data (SQLite):")
